...
function SkeletonMsg({ isUser }) {
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-4`}>
      {!isUser && (
        <div className="w-8 h-8 mr-2 rounded-xl skeleton flex-shrink-0" />
      )}
...
/* Burn fuse typing indicator — flickering flame edition */
function FuseTyping({ persona }) {
  return (
    ...
    >
      ...
      <div className="liquid-raised px-4 py-3 rounded-2xl rounded-tl-sm flex items-center gap-1.5">
        <span className="flame-1">🔥</span>
        <span className="flame-2">🔥</span>
        <span className="flame-3">🔥</span>
        <span className="text-sm text-gray-300 ml-2 font-mono capitalize tracking-wide">
          {persona} is loading…
        </span>
      </div>
    </motion.div>
...
function ChatBubble({ role, content, persona }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`max-w-[85%] sm:max-w-[80%] ${!isUser && "flex flex-col sm:flex-row gap-3 sm:gap-4 items-start"}`}
      >
        <div className="flex-1">
          ...
          <div
            className={`px-5 py-4 rounded-2xl text-base leading-relaxed whitespace-pre-wrap shadow-lg ${
              isUser
                ? "bubble-user rounded-br-sm text-gray-100"
                : "bubble-ai rounded-bl-sm text-gray-100 font-medium"
            }`}
          >
            {content ||
              (!isUser && (
                <span className="text-gray-400 cursor-blink font-mono">▌</span>
              ))}
          </div>
        </div>
      </div>
...
export default function ChatWindow({ persona, mode }) {
  ...
  const currentChat = chats[persona] || {
    messages: [],
    usedAngles: [],
    conversationId: null,
  };
  ...
  useEffect(() => {
    async function initOrRestoreChat() {
      ...
      try {
        ...
        let historyMessages = Array.isArray(historyRes.messages)
          ? historyRes.messages
          : [];
          ...
          return {
            ...
            [persona]: {
              ...
              messages: current.messages.map((m) =>
                m.id === typingMsg.id ? { ...m, content: accumulated } : m,
              ),
            },
          };
        });
        ...
        setChats((prev) => {
          ...
          return {
            ...
            [persona]: {
              ...current,
              messages: current.messages.map((m) =>
                m.id === typingMsg.id ? { ...m, content: fullText } : m,
              ),
            },
          };
        });
  ...
  <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-5 pb-20">
        {messages.length === 0 && (
          ...
          >
            ...
            <h3 className="font-display font-black text-5xl text-white/90 mb-4">
              The Hot Seat
            </h3>
