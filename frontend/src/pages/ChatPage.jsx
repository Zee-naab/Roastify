    ...
    <div className="h-screen flex flex-col bg-bg overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 py-3 glass border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <span className="font-display font-black text-xl tracking-tight text-heat-gradient">Roastify</span>
          {selectedCeleb && (
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl glass text-sm text-white/70 border border-white/5">
              ...
              <span className="capitalize font-medium text-white/90">{selectedCeleb.name}</span>
              <span className="text-white/20">·</span>
              <span className="capitalize text-heat text-xs font-semibold">{mode}</span>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {sidebarOpen && (
          <aside className="w-64 sm:w-72 flex-shrink-0 border-r border-white/5 overflow-y-auto p-4 flex flex-col gap-6 bg-surface/50">
            <section>
              <p className="text-xs font-semibold uppercase tracking-widest text-muted mb-3">Celebrity</p>
