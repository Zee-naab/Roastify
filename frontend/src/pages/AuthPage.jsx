...
export default function AuthPage() {
    ...
    try {
      if (isLogin) {
        ...
        if (data.message || data.success) {
          ...
          setError(data.error || 'Could not create account. Try signing in instead.');
        }
      }
    } catch {
  ...
  /* ── Headings ── */
  const heading = step === 'otp'
    ? 'Check your inbox.'
    : isLogin ? 'Welcome back.' : 'Get your backstage pass.';
    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none" aria-hidden>
        <motion.div
          ...
          style={{ background: 'radial-gradient(circle, rgba(138,43,226,0.2) 0%, transparent 65%)' }}
          animate={{ opacity: [0.6, 1, 0.6] }}
          transition={{ duration: 6, repeat: Infinity }}
        />
        <motion.div
          className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(255,69,0,0.15) 0%, transparent 65%)' }}
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-display font-black text-3xl tracking-tight mb-1 text-white">{heading}</h1>
        ...
        <div className="liquid-raised rounded-2xl p-7 plasma-border">
          <AnimatePresence mode="wait">
            ...
            {step === 'otp' ? (
              ...
              >
                <div>
                  <label htmlFor="auth-otp" className="block text-sm font-semibold uppercase tracking-widest text-gray-400 mb-2">
                    Your 6-digit code
                  </label>
                  <input
                    id="auth-otp"
                ...
                {error && (
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-red-400 text-sm bg-red-400/10 px-3 py-2 rounded-lg border border-red-400/20">
                    {error}
                  </motion.p>
                )}
                <button
              ...
              >
                {/* Email */}
                <div>
                  <label htmlFor="auth-email" className="block text-sm font-semibold uppercase tracking-widest text-gray-400 mb-2">
                    Email address
                  </label>
                  <input
                    id="auth-email"
                ...
                {/* Password */}
                <div>
                  <label htmlFor="auth-password" className="block text-sm font-semibold uppercase tracking-widest text-gray-400 mb-2">
                    Password
                  </label>
                  <input
                    id="auth-password"
                ...
                <button type="submit" disabled={loading} className="btn-plasma w-full py-3.5 rounded-xl text-base font-semibold">
                  {loading
        ...
        <div className="mt-5 text-center">
          ...
          <p className="text-xs text-gray-600 mt-3 font-mono">
            {isLogin ? 'Returning members sign in with password.' : 'New members verify once via email.'}
          </p>
        </div>
      </motion.div>
    </div>
